...
class gomovies(Scraper):
    def __init__(self):
        ...
        self.bu = 'https://0gomovies.app/genre/'
    ...
    def get_videos(self, url):
        ...
        if 'ZZZZ' in url:
            ...
            html = client.request(url, verify=False)
            mlink = SoupStrainer('div', {'class': 'ch_btn_box'})
            videoclass = BeautifulSoup(html, "html.parser", parse_only=mlink)
            links = videoclass.find_all('div', {'class': 'chbox'})
            for link in links:
                iurl = link.find('button').get('onclick')
                if 'link=' in iurl:
                    iurl = iurl.split('link=')[-1]
                if 'url=' in iurl:
                    iurl = iurl.split('url=')[-1]
                iurl = iurl[:-2]
                if '/hls/' in iurl:
