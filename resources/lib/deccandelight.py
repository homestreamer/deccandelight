...
sites = {'01tgun': 'Tamil Gun : [COLOR yellow]Tamil[/COLOR]',
         ...
         '41ibomma': 'iBOMMA : [COLOR magenta]Various[/COLOR]',
...
def list_items(site, iurl):
    ...
    for movie in movies:
        ...
        if 'Next Page' in title:
            ...
            url = '{0}?action={1}&site={2}&title={3}&thumb={4}&iurl={5}'.format(control._url, mode, site, qtitle, urllib_parse.quote(mthumb), iurl)
            fanart = control._fanart
            poster = movie[1]
...
def play_video(vid_url, dl=False):
    ...
    if dl and stream_url:
        ...
        # non_ia = ['yupp', 'SUNNXT', 'tamilgun', 'vidmojo', 'serafim', '__temp_', 'cloud', 'tvlogy', 'gomovies', 'xyz']
        # xbmc.log('\n@@@@DD Final URL = {}\n'.format(stream_url), xbmc.LOGNOTICE)

        if control.kodiver >= 17.0:  # and not any(x in stream_url for x in non_ia):

            if '.m3u8' in stream_url or '/m3u8/' in stream_url:
                play_item.setMimeType('application/vnd.apple.mpegstream_url')
                play_item.setContentLookup(False)
                adaptive_list = ['master', 'adaptive', 'tamilray', 'index', 'playallu']
